...
class LexicalDocument:
    ...
    def tokens(self) -> list[str]:
        return sorted(set(_tokenize(self.searchable_text)))
...
def delete_file(conn, project: str, file_id: int) -> None:
    ensure_schema(conn)
    conn.execute("DELETE FROM lexical_terms WHERE project_name = ? AND doc_id IN (SELECT id FROM lexical_documents WHERE project_name = ? AND file_id = ?)", (project, project, file_id))
    conn.execute("DELETE FROM lexical_documents WHERE project_name = ? AND file_id = ?", (project, file_id))


def _insert_document(conn, document: LexicalDocument) -> None:
    ensure_schema(conn)
...
def sync_analysis(
    ...
    *,
    ...
    _insert_document(conn, file_document)

    for order, symbol in enumerate(analysis.get("symbols", [])):
        sqlite_uri = _sqlite_symbol_uri(project, file_id, order)
        ...
        _insert_document(conn, symbol_document)
...
def search(
    ...
    *,
    ...
    ensure_schema(conn)
    query_terms, query_text = _normalize_query(query)
    ...
    rows = conn.execute(
        f"SELECT * FROM lexical_documents {where_sql}",
        tuple(params),
    ).fetchall()
    ...
    for row in rows:
        document = dict(row)
        score = _score_document(document, query_terms, query_text)
        if score <= 0:
            continue
