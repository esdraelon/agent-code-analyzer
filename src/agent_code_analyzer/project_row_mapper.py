...
class ProjectRowMapper:
    ...
    def file_record(row: sqlite3.Row) -> dict[str, Any]:
        return {
            ...
            "languages": ProjectRowMapper.decode_languages(row["languages"] if "languages" in row.keys() else "[]"),
            "root_type": row["root_type"] if "root_type" in row.keys() else "",
            "start_point": {"row": row["root_start_row"] if "root_start_row" in row.keys() else 0, "column": row["root_start_column"] if "root_start_column" in row.keys() else 0},
            "end_point": {"row": row["root_end_row"] if "root_end_row" in row.keys() else 0, "column": row["root_end_column"] if "root_end_column" in row.keys() else 0},
